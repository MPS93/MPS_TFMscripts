...
load "../functions.m";
//load Ew2Ed_symb, Ed2Ew_symb, Ew2Ed_eval, Ed2Ew_eval


printf "\n\n MD BIRATIONAL EQUIVALENCE BETWEEN Ed AND Ew \n\n";
...
x := Evaluate(Ew2Ed_symb(BF,[a1,a2,a3,a4,a6])[2][1],[u,v]);
y := Evaluate(Ew2Ed_symb(BF,[a1,a2,a3,a4,a6])[2][2],[u,v]);

print "\n Change of coordinates from Ew to EdC: \n";
" x := ", x; "\n y := ", y;


BF := Rationals();
R<a1,a2,a3,a4,a6,d> := FunctionField(BF,6);
Rad_var<x,y> := FunctionField(R,2);
u := Evaluate(Ed2Ew_symb(BF,[d])[2][1],[x,y]);
v := Evaluate(Ed2Ew_symb(BF,[d])[2][2],[x,y]); 

print "\n Change of coordinates from EdC to Ew: \n";
" u := ", u; "\n v := ", v;
...
"\n The given Ew above and its point p_w := ", Rw_var!p, " correspond to the following EdC and p_ad \n";

ch := Ew2Ed_eval(BF,lst,p);

R<a1,a2,a3,a4,a6,d> := FunctionField(BF,6);
Rad_var<x,y> := AffineSpace(R,2);
d := ch[1][1];
C := Curve(Rad_var, 1 + d*x^2*y^2 - x^2 - y^2); C;
p_ad := Rad_var![ch[2][1],ch[2][2]]; "\n p_ad := ", p_ad;
tf,pt := p_ad in C; "\n Is p_ad in the EdC? \n\t", tf;
...
"\n The given EdC above and its point p_ad := ", Rad_var!p, " correspond to the following Ew and p_w \n";

ch := Ed2Ew_eval(BF,lst,p);
R<a1,a2,a3,a4,a6,d> := FunctionField(BF,6);
Rw_var<u,v> := AffineSpace(R,2);
...
BF := FiniteField(p);
R<a1,a2,a3,a4,a6,d> := FunctionField(BF,6);
R2<x,y> := AffineSpace(R,2);
d := -39081; lst := [d];
C := Curve(R2, 1 + d*x^2*y^2 - x^2 - y^2); C;
p := R2 ! [117812161263436946737282484343310064665180535357016373416879082147939404277809514858788439644911793978499419995990477371552926308078495, 19];

"\n The given EdC above and its point p_ad := ", R2!p, " correspond to the following Ew and p_w \n";
...
ch := Ew2Ed_eval(BF,lst,p: PP4 := PP4_l);
...
ch := Ed2Ew_eval(BF,lst_d,p_ad : PP4 := PP4_l, a1 := a1, a3 := a3);
a1 := ch[1][1]; a2 := ch[1][2]; a3 := ch[1][3]; a4 := ch[1][4]; a6 := ch[1][5]; 
C := Curve(Rw_var, v^2 + a1*u*v + a3*v - u^3 - a2*u^2 - a4*u - a6); C;
p_w := Rw_var![ch[2][1],ch[2][2]]; "\n p_w := ", p_w;
tf,pt := p_w in C; "\n Is p_w in the Ew? \n\t", tf;
...
"\n The given EdC above and its point p_ad := ", Rd_var!p, " correspond to the following Ew and p_w \n";

ch := Ed2Ew_eval(BF,lst,p : a1 := 724, a3 := 6705);
a1 := ch[1][1]; a2 := ch[1][2]; a3 := ch[1][3]; a4 := ch[1][4]; a6 := ch[1][5]; 
C := Curve(Rw_var, v^2 + a1*u*v + a3*v - u^3 - a2*u^2 - a4*u - a6); C;
p_w := Rw_var![ch[2][1],ch[2][2]]; "\n p_w := ", p_w;
tf,pt := p_w in C; "\n Is p_w in the Ew? \n\t", tf;
