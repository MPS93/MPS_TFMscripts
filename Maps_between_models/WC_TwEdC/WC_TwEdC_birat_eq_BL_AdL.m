//*** Checking that addition is preserved through the BL birat. eq. ***//

load "../functions.m";
//load ProjCoord, AffCoord, Ew2Ead_P4_eval, Ead2Ew_P4_eval, Ead_AddL_eval, Ew_AddL_eval


printf "\n ADDITION PRESERVATION THROUGH BL \n";
...
p_ad := Ew2Ead_P4_eval(BF,lst_w,P4,p_w);
q_ad := Ew2Ead_P4_eval(BF,lst_w,P4,q_w);
pq_ad := Ew2Ead_P4_eval(BF,lst_w,P4,pq_w);
...
printf "\n Using the Ew2Ead birational equivalence, we have that these points go to: \n\t p_ad = (%o,%o) \n\t q_ad = (%o, %o) \n\t pq_ad = (%o, %o)\n", p_ad[1], p_ad[2], q_ad[1], q_ad[2], pq_ad[1], pq_ad[2];
...
p_ad := Ew2Ead_P4_eval(BF,lst_w,P4,p_w: a := 25);
q_ad := Ew2Ead_P4_eval(BF,lst_w,P4,q_w: a := 25);
pq_ad := Ew2Ead_P4_eval(BF,lst_w,P4,pq_w: a := 25);
lst_ad := p_ad[2]; a:= lst_ad[1]; d := lst_ad[2];
p_ad := [p_ad[3][1],p_ad[3][2]];
q_ad := [q_ad[3][1],q_ad[3][2]];
pq_ad := [pq_ad[3][1],pq_ad[3][2]];
...
printf "\n\n\n The same Ew can also be birat. equivalent to a Ead with a != 1. So using the same Ew and the same points on it, let's see how the Ew2Ead birational equivalence works for a = 25.\n";
...
p_ad := Ew2Ead_P4_eval(BF,lst_w,P4,p_w: a := 5);
BF := p_ad[1]; 
R<a1,a2,a3,a4,a6,up,vp,d> := FunctionField(BF,8);
Rd_var<x,y> := AffineSpace(R,2);
Rw<X1,Y1,Z1,T1, X2,Y2,Z2,T2> := PolynomialRing(BF,8);
q_ad := Ew2Ead_P4_eval(BF,lst_w,P4,q_w: a := 5);
pq_ad := Ew2Ead_P4_eval(BF,lst_w,P4,pq_w: a := 5);
lst_ad := p_ad[2]; a:= lst_ad[1]; d := lst_ad[2];
...
printf "\n\n\n What happens if 'a' is not a square on BF? Well, then we have to work on the algebraic closure of BF. So using the same Ew and the same points on it, let's see how the Ew2Ead birational equivalence works for a = 5.\n";
printf "\n We have that p_w, q_w and pq_w go to: \n\t p_ad = (%o,%o) \n\t q_ad = (%o, %o) \n\t pq_ad = (%o, %o)\n", p_ad[1], p_ad[2], q_ad[1], q_ad[2], pq_ad[1], pq_ad[2];
...
p_ad := Ew2Ead_P4_eval(BF,lst_w,P4,p_w: a := 3);
q_ad := Ew2Ead_P4_eval(BF,lst_w,P4,q_w: a := 3);
pq_ad := Ew2Ead_P4_eval(BF,lst_w,P4,pq_w: a := 3);
lst_ad := p_ad[2]; a:= lst_ad[1]; d := lst_ad[2];
p_ad := [p_ad[3][1],p_ad[3][2]];
q_ad := [q_ad[3][1],q_ad[3][2]];
pq_ad := [pq_ad[3][1],pq_ad[3][2]];
...
p_ad := Ew2Ead_P4_eval(BF,lst_w,P4,p_w: a := 11);
BF := p_ad[1];
R<a1,a2,a3,a4,a6,up,vp,d> := FunctionField(BF,8);
Rd_var<x,y> := AffineSpace(R,2);
Rw<X1,Y1,Z1,T1, X2,Y2,Z2,T2> := PolynomialRing(BF,8);
q_ad := Ew2Ead_P4_eval(BF,lst_w,P4,q_w: a := 11);
pq_ad := Ew2Ead_P4_eval(BF,lst_w,P4,pq_w: a := 11);
lst_ad := p_ad[2]; a:= lst_ad[1]; d := lst_ad[2];
p_ad := [BF|p_ad[3][1],p_ad[3][2]];
q_ad := [BF|q_ad[3][1],q_ad[3][2]];
pq_ad := [BF|pq_ad[3][1],pq_ad[3][2]];
...
printf "\n\n\n What happens if 'a' is not a square on BF? Well, then we have to work on the algebraic closure of BF. So using the same Ew and the same points on it, let's see how the Ew2Ead birational equivalence works for a = 11.\n";
printf "\n We have that p_w, q_w and pq_w go to: \n\t p_ad = (%o,%o) \n\t q_ad = (%o, %o) \n\t pq_ad = (%o, %o)\n", p_ad[1], p_ad[2], q_ad[1], q_ad[2], pq_ad[1], pq_ad[2];
printf "\n Here 'r1' represents the square root of 'a'. And the BF has been changed to: \n\t %o \n", BF;
printf "\n These points are supposed to be points over the Ead with params a= %o, d = %o. Are they really on this curve? \n\t p_ad -> %o, q_ad -> %o, pq_ad -> %o \n", a, d, Rd_var!p_ad in Cad, Rd_var!q_ad in Cad, Rd_var!pq_ad in Cad;
printf "\n Now it's time to add p_ad and q_ad with the addition law for twisted Edwards curves: \n\t p_ad + q_ad = r_ad = (%o, %o) \n", r_ad[1], r_ad[2];
...
P4 := [BF|67255, 4925250774549309901534880012517951725634967408808180833493536675530715221437151326426783281860614455100818821787429];

p_w := Ead2Ew_P4_eval(BF,lst_ad,P4,p_ad : a1 := 287765, a3 := 97646);  
q_w := Ead2Ew_P4_eval(BF,lst_ad,P4,q_ad : a1 := 287765, a3 := 97646)[3]; q_w := [BF|q_w[1],q_w[2]]; Q_w := [q_w[1],q_w[2],1];
pq_w := Ead2Ew_P4_eval(BF,lst_ad,P4,pq_ad : a1 := 287765, a3 := 97646)[3]; pq_w := [BF|pq_w[1],pq_w[2]];
lst_w := p_w[2]; p_w := [BF|p_w[3][1],p_w[3][2]]; P_w := [p_w[1],p_w[2],1];
